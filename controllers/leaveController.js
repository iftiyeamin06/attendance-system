...
// Admin-driven leave creation (creates Approved leave and syncs attendance for full-day leaves)
async function createLeave(req, res) {
  try {
    ...
    if (!user_id || !start_date || !end_date || !leave_type) {
      return res.status(400).json({ success: false, message: 'user_id, start_date, end_date, and leave_type are required.' });
    }

    if (leave_type === 'partial' && (!partial_from || !partial_to)) {
      return res.status(400).json({ success: false, message: 'partial_from and partial_to are required for partial leaves.' });
    }
    ...
    if (leave_type !== 'partial') await syncLeaveToAttendance(leave);

    return res.status(201).json({ success: true, message: 'Leave recorded and approved.', data: { id: leave.id, status: leave.status } });
  } catch (err) {
    console.error('Create leave (admin) error:', err);
    return res.status(500).json({ success: false, message: 'An error occurred.' });
  }
}

// Employee-facing: submit a leave request (status Pending)
async function submitLeaveRequest(req, res) {
  try {
    ...
    if (!start_date || !end_date || !leave_type) {
      return res.status(400).json({ success: false, message: 'start_date, end_date, and leave_type are required.' });
    }
    ...
    return res.status(201).json({ success: true, message: 'Leave request submitted.', data: { id: leave.id, status: leave.status } });
  } catch (err) {
...
async function updateLeaveStatus(req, res) {
  try {
    ...
    if (!['Approved', 'Rejected'].includes(status)) return res.status(400).json({ success: false, message: 'Invalid status. Use Approved or Rejected.' });
    ...
    if (status === 'Approved' && leave.leaveType !== 'partial') {
      await syncLeaveToAttendance(leave);
    }

    if (status === 'Rejected' && leave.leaveType !== 'partial') {
    ...
    return res.json({ success: true, message: `Leave ${status.toLowerCase()}.`, data: { id: leave.id, status: leave.status } });
  } catch (err) {
...
async function getLeaves(req, res) {
  try {
    ...
    const leaves = await Leave.findAll({ where, include: [{ model: User, as: 'user', attributes: ['id', 'name', 'email'] }], order: [['startDate', 'DESC']] });

    const officeTimes = await getOfficeTimes();

    return res.json({ success: true, data: leaves.map((l) => {
      let partialInfo = null;
      if (l.leaveType === 'partial') partialInfo = detectPartialLeaveType(l.partialFrom, l.partialTo, officeTimes.start, officeTimes.end);
      return {
        id: l.id,
        user_id: l.userId,
...
async function syncLeaveToAttendance(leave) {
  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    const existing = await AttendanceLog.findOne({ where: { userId: leave.userId, clockInTime: { [Op.gte]: dayStart, [Op.lte]: dayEnd } } });

    if (!existing) {
      await AttendanceLog.create({ userId: leave.userId, clockInTime: null, ipAddress: 'N/A', deviceIdUsed: 'N/A', status: 'ON_LEAVE' });
    }
  }
}

module.exports = { createLeave, submitLeaveRequest, updateLeaveStatus, deleteLeave, getLeaves, syncLeaveToAttendance, detectPartialLeaveType, getOfficeTimes, timeToMinutes };
