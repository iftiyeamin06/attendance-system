...
async function runTests() {
  ...
  // Mailer Unit & Integration Tests (mocked SMTP)
  ...
  // 1. Fallback when SMTP environment variables are missing
  delete process.env.SMTP_HOST;
  delete process.env.SMTP_USER;
  const fallbackRes = await mailer.sendResetEmail('fallback@attendance.local', 'http://localhost:3000/reset-password?token=test1234');
  test('Mailer: graceful fallback when SMTP variables missing',
    fallbackRes.success === false && fallbackRes.fallback === true,
    JSON.stringify(fallbackRes));

  // 2. Mocked transporter sends HTML email with reset URL and 15-minute notice
  process.env.SMTP_HOST = 'smtp.test.local';
  process.env.SMTP_PORT = '587';
  process.env.SMTP_USER = 'test-mailer@attendance.local';
  process.env.SMTP_PASS = 'mockpassword';
  process.env.SMTP_FROM = '"Attendance System" <no-reply@attendance.local>';
  ...
  nodemailer.createTransport = function () {
    return {
      sendMail: async function (mailOptions) {
        ...
        return { messageId: '<test-message-id@local>' };
      },
    };
  };

  const mockSendRes = await mailer.sendResetEmail(
    'employee@attendance.local',
    'http://localhost:3000/reset-password?token=mocktoken123'
  );

  test('Mailer: sends HTML email with reset URL and 15-minute expiry',
    mockSendRes.success === true &&
    mockSendRes.messageId === '<test-message-id@local>' &&
    sentMailOptions?.to === 'employee@attendance.local' &&
    sentMailOptions?.subject === 'Password Reset - Attendance System' &&
    sentMailOptions?.html?.includes('http://localhost:3000/reset-password?token=mocktoken123') &&
    sentMailOptions?.html?.includes('15 minutes'),
    `Subject: ${sentMailOptions?.subject}, To: ${sentMailOptions?.to}`);

  // 3. Error handling when sendMail rejects
  nodemailer.createTransport = function () {
    return {
  ...
  const mockErrorRes = await mailer.sendResetEmail(
    'employee@attendance.local',
    'http://localhost:3000/reset-password?token=errortoken123'
  );
  test('Mailer: handles sendMail errors gracefully without throwing',
    mockErrorRes.success === false && mockErrorRes.error === 'SMTP connection failed',
    JSON.stringify(mockErrorRes));

  // Restore createTransport
  nodemailer.createTransport = origCreateTransport;

  // 4. Production guardrail: reset_token and reset_url stripped in production mode
  process.env.NODE_ENV = 'production';
  let prodPayload = null;
  const mockReq = {
    body: { email: pwEmail },
    protocol: 'https',
    get: () => 'attendance.company.com',
    socket: { remoteAddress: '127.0.0.1' },
    ip: '127.0.0.1',
  };
  const mockRes = {
    status(c) { this.statusCode = c; return this; },
    json(p) { prodPayload = p; return this; },
  };
  await requestPasswordReset(mockReq, mockRes);
  test('Security guardrail: reset_token and reset_url stripped in production',
    prodPayload?.success === true &&
    !prodPayload?.reset_token &&
    !prodPayload?.reset_url &&
    prodPayload?.message === 'If that email exists, a password reset link has been sent to your inbox.',
    JSON.stringify(prodPayload));

  // Restore environment variables
  if (origSmtpHost) process.env.SMTP_HOST = origSmtpHost; else delete process.env.SMTP_HOST;
  if (origSmtpUser) process.env.SMTP_USER = origSmtpUser; else delete process.env.SMTP_USER;
  if (origSmtpPass) process.env.SMTP_PASS = origSmtpPass; else delete process.env.SMTP_PASS;
  if (origSmtpPort) process.env.SMTP_PORT = origSmtpPort; else delete process.env.SMTP_PORT;
  if (origSmtpFrom) process.env.SMTP_FROM = origSmtpFrom; else delete process.env.SMTP_FROM;
